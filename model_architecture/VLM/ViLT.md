...
视觉-语言模型的四种范式：每个矩形的高度表示它的相对计算大小。VE、TE 和 MI 分别代表视觉编码器、文本编码器和模态交互器
...
class ViLT(nn.Module):
    ...
    def forward(self, images, texts):
        ...
        cls_feats = self.tanh(self.pooler(cls_output))

        return image_feats, text_feats, cls_feat
```
...
以0.5的概率将对齐后的图像随机替换为不同的图像。单个线性层ITM头将Pooler的输出特征`cls_feat`投影到二分类上，并且计算`negative log-likelihood loss `作为ITM损失

## Masked Language Modeling

目标是从其上下文向量`text_feats`中预测masked文本的真实标签
